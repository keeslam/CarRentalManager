...
export async function registerRoutes(app: Express): Promise<void> {
  ...
  app.patch("/api/pdf-templates/:id", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      // Generate thumbnail preview of the template (async, don't wait)
      (async () => {
        try {
          const { generateRentalContractFromTemplate } = await import('./utils/pdf-generator');
          ...
          // Create dummy reservation for preview
          const dummyReservation: Reservation = {
            id: 0,
            vehicleId: 0,
            customerId: 0,
          ...
          // Generate PDF
          const pdfBuffer = await generateRentalContractFromTemplate(dummyReservation, updatedTemplate);
          
          // Save temporary PDF
          const tempDir = 'uploads/templates/temp';
          const previewDir = 'uploads/templates/previews';
          if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir, { recursive: true });
          if (!fs.existsSync(previewDir)) fs.mkdirSync(previewDir, { recursive: true });
          
          const tempPdfPath = path.join(tempDir, `template_${id}_temp.pdf`);
          const previewPngPath = path.join(previewDir, `template_${id}_preview.png`);
          
          fs.writeFileSync(tempPdfPath, pdfBuffer);
          
          // Convert to PNG with scale 1 for smaller preview
          await convertPdfToPng(tempPdfPath, previewPngPath, 1);
          
          // Update template with preview path
          await storage.updatePdfTemplate(id, { templatePreviewPath: previewPngPath });
          
          // Clean up temp PDF
          fs.unlinkSync(tempPdfPath);
          
          console.log(`✅ Generated thumbnail preview for template ${id}`);
        } catch (error) {
